 ...
 *
 * 但我们提到并发、多线程就离不开线程池的使用，因为线程池能够复用线程，减少线程的频繁创建与销毁，如果使用InheritableThreadLocal，
 * 那么线程池中的线程拷贝的数据来自于第一个提交任务的外部线程，即后面的外部线程向线程池中提交任务时，子线程访问的本地变量都来源于第一个外部线程，造成线程本地变量混乱
 */
public class TransmittableThreadLocalTest {
    ...
    /*** 业务线程池，默认Control中异步任务执行线程池 使用ttl线程池，该框架的使用，请查阅官方文档。*/
    private static ExecutorService businessExecutors = TtlExecutors.getTtlExecutorService(Executors.newFixedThreadPool(4));
    /**
     * 线程上下文环境，模拟在Control这一层，设置环境变量，然后在这里提交一个异步任务，模拟在子线程中，是否可以访问到刚设置的环境变量值。
     */
    private static TransmittableThreadLocal<Integer> requestIdTransmittableThreadLocal = new TransmittableThreadLocal<>();

    public static void main(String[] args) {
        for (int i = 0; i < 10; i++) {
            // 模式10个请求，每个请求执行ControlThread的逻辑，其具体实现就是，先输出父线程的名称，
            //  然后设置本地环境变量，并将父线程名称传入到子线程中，在子线程中尝试获取在父线程中的设置的环境变量 21
    ...
    static class ControlThread implements Runnable {
        ...
        public void run() {
            ...
            requestIdTransmittableThreadLocal.set(i);
            //使用线程池异步处理任务
    ...
    static class BusinessTask implements Runnable {
        ...
        public void run() {
            ...
            System.out.println("parentThreadName:" + parentThreadName + ":" + requestIdTransmittableThreadLocal.get());
        }
    }
